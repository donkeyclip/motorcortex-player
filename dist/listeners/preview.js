...
module.exports = _this => {
  ...
  if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    const loopBarMouseInOut = () => {
      ...
      elid(`${_this.name}-hover-display`).classList.toggle(`${_this.name}-hide`);
    ...
    const loopBarMouseMove = e => {
      ...
      if (clientX - viewportOffset.left + _this.settings.loopLastPositionXPxls > _this.settings.loopLastPositionXPxls + _this.elements.loopBar.offsetWidth && !_this.settings.resizeLoop) {
        elid(`${_this.name}-hover-millisecond`).innerHTML = _this.settings.loopEndMillisecond;
        return;
      } else if (clientX - viewportOffset.left < 0 && !_this.settings.resizeLoop) {
        elid(`${_this.name}-hover-millisecond`).innerHTML = _this.settings.loopStartMillisecond;
        return;
      }

      let positionX = clientX - viewportOffset.left + _this.settings.loopLastPositionXPxls;

      if (positionX < 0) {
        positionX = 0;
      }

      let left = positionX - elid(`${_this.name}-hover-display`).offsetWidth / 2;

      if (left < 0) {
        left = 0;
      } else if (left + elid(`${_this.name}-hover-display`).offsetWidth > _this.elements.totalBar.offsetWidth) {
        left = _this.elements.totalBar.offsetWidth - elid(`${_this.name}-hover-display`).offsetWidth;
      }

      const ms = Math.round(positionX / _this.elements.totalBar.offsetWidth * _this.clip.duration);
      if (_this.options.preview) {

...
module.exports = function (_this) {
  _this.elements = {};
  var clipIframe = _this.clip.props.host.getElementsByTagName("iframe")[0];
  ...
  _this.elements.mcPlayer.innerHTML = playerHTML({ svg: svg, name: _this.name });
  ...
  _this.elements.settingsPointerEvents = elid(_this.name + "-settings-pointer-events");
  _this.elements.speedBarHelper = elid(_this.name + "-speed-value-helperbar");
  _this.elements.settingsShowIndicator = elid(_this.name + "-settings-indicator");
  _this.elements.settingsSpeedButtonShow = elid(_this.name + "-settings-speed-show");
  _this.elements.settingsSpeedButtonHide = elid(_this.name + "-settings-speed-hide");
  ...
  _this.elements.editableLoopStartTime.size = elid(_this.name + "-time-total").innerHTML.length + 1;

  _this.elements.editableLoopStartTime.maxLength = elid(_this.name + "-time-total").innerHTML.length;

  _this.elements.editableLoopStartTime.style.height = elid(_this.name + "-time-total").offsetHeight;

  _this.elements.editableLoopStartTime.value = elid(_this.name + "-loopbar-start-time").innerHTML;
  ...
  _this.elements.editableLoopEndTime.size = elid(_this.name + "-time-total").innerHTML.length + 1;

  _this.elements.editableLoopEndTime.maxLength = elid(_this.name + "-time-total").innerHTML.length;

  _this.elements.editableLoopEndTime.style.height = elid(_this.name + "-time-total").offsetHeight;

  _this.elements.editableLoopEndTime.value = elid(_this.name + "-loopbar-start-time").innerHTML;
  ...
  elid(_this.name + "-pointer-events-checkbox").checked = _this.options.pointerEvents;
  ...
  if (!_this.options.showVolume) {
    ...
    _this.elements.volumeControl.classList.toggle(_this.name + "-volume-width-transition");
